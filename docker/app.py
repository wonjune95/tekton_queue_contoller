...
# Dashboard SA만 큐를 통한 실행 허용. 그 외 출처(배스천 등)는 Pending으로만 설정되며
# managed 라벨이 없으므로 매니저가 스케줄링하지 않는다.
...
# Webhook 통과 후 Watcher 캐시에 반영되기까지의 공백을 메우는 in-flight 카운터.
# 캐시의 running_cnt와 합산하여 쿼터를 판정한다.
...
METRIC_WEBHOOK_QUEUED = Counter('tekton_queue_webhook_queued_total', 'Dashboard PR이 쿼터 초과로 대기열로 보내진 횟수', ['tier'])
METRIC_WEBHOOK_HELD = Counter('tekton_queue_webhook_held_total', 'Dashboard 외 출처에서 생성되어 보류된 횟수', ['tier'])
...
def _ensure_admitted_configmap():
    try:
        core_api.create_namespaced_config_map(
            LEASE_NAMESPACE,
            client.V1ConfigMap(
                metadata=client.V1ObjectMeta(name=ADMITTED_CM_NAME, namespace=LEASE_NAMESPACE),
                data={'admitted': '0'}
            )
        )
...
def _try_increment_global_admitted(running_cnt, limit, max_retries=5):
...
def _decrement_global_admitted(max_retries=5):
    global webhook_admitted_count
    for attempt in range(max_retries):
        try:
            cm = core_api.read_namespaced_config_map(ADMITTED_CM_NAME, LEASE_NAMESPACE)
...
def determine_tier(labels, tier_rules):
    for rule in tier_rules:
        ...
        if match_type == 'label':
            ...
            # env 라벨이 없어도 fnmatch('', '*') == True 이므로 catch-all 규칙이 정상 매칭된다.
...
def get_queue_status_from_cache():
    # cache_lock 진입 전에 config를 읽어 lock 중첩(데드락)을 방지한다.
    cfg = get_cached_config()
    aging_interval = cfg["aging_interval_sec"]
    aging_min = cfg["aging_min_tier"]
    ...
    def get_priority_and_time(item):
        ...
        # tier-0(긴급)처럼 이미 aging_min보다 낮은 경우 aging으로 인해 더 낮아지지 않도록 보장한다.
...
def update_cache(event_type, obj):
    ...
    with cache_lock:
        ...
        # resourceVersion='__admitted__'는 Webhook이 즉시 실행 허용 시 삽입하는 임시 항목이다.
        # 실제 K8s 이벤트가 도착하면 교체하며 이 시점에 webhook_admitted_count를 감소시킨다.
...
def mutate_pipelinerun():
    ...
    if not is_from_dashboard:
        METRIC_WEBHOOK_HELD.labels(tier=str(tier_val)).inc()
    ...
    # Dashboard PR: 캐시 미동기화 시 running_cnt=0으로 보여 쿼터를 무시하고 통과되므로
    # 초기 동기화가 완료된 이후에만 쿼터 판정을 수행한다.
    if not initial_sync_done:
    ...
    # generateName PR은 Webhook 시점에 이름이 확정되지 않아 phantom key가 Watcher key와
    # 달라 캐시에 영구 잔존하는 문제가 생기므로 name이 확정된 PR에만 phantom을 삽입한다.
        ...
        try:
            ...
            try:
                ...
                if e.status == 404:
                    lease_body = client.V1Lease(
                        metadata=client.V1ObjectMeta(name=LEASE_NAME, namespace=LEASE_NAMESPACE),
            ...
            if e.status == 409:
                if _is_renewal_attempt:
                    # 갱신 충돌은 일시적이므로 is_leader를 유지하고 재시도한다.
                    # 즉시 해제하면 네트워크 지연마다 스케줄링 공백이 발생한다.
    ...
    while True:
        try:
            ...
            if e.status == 410:
                ...
                log(f"[Watcher] API 에러 ({e.status}): {e.reason}. 기존 위치에서 재연결 시도 중...")
            time.sleep(2)
        except Exception as e:
